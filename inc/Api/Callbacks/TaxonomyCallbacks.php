...
class TaxonomyCallbacks
{
	...
	{
		$output = get_option('getweb_plugin_tax');
	...
	public function checkboxPostTypesField( $args )
	{
		...
		foreach ($post_types as $post) {
			...
			$output .= '<div class="' . $classes . ' mb-10"><input type="checkbox" id="' . $post . '" name="' . $option_name . '[' . $name . '][' . $post . ']" value="1" class="" ' . ( $checked ? 'checked' : '') . '><label for="' . $post . '"><div></div></label> <strong>' . ucfirst($post) . '</strong></div>';
		}
